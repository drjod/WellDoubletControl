...
{
        Q_H -= fabs(Q_w) * scheme->simulator->get_heatcapacity() * (
...
{
        if(scheme->operationType == WellDoubletControl::storing)
                ...
                Q_w = fmax(Q_H / (scheme->simulator->get_heatcapacity()  *
                        (T1 + T2 - 2 * scheme->value_target)), scheme->value_threshold);
...
void WellDoubletCalculation::adapt_flowrate(WellSchemeAC* scheme)
{
	double delta = 4. * ((this->*simulation_result_aiming_at_target)() - 
			scheme->value_target) / fabs(scheme->value_target);
	...
	{
		Q_w = fmin(Q_w * (1 - delta), 0.);  // else storing
		Q_w = fmax(Q_w, scheme->value_threshold);
	}
